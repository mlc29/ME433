...
def webpage():
    ...
    <p><button class="sw_button" name="Stopwatch" type="submit">Stopwatch</button></a></p>
    <br>
    <p class="dotted">Time since last stopwatch click is
    <span style="color: deeppink;"><span id="buttonpress">{temp_time}</span></span></p>
    </body>
    <script>
    // LED buttons handler
    ...
    $(".sw_button").click((el) => {{ $.post("/Stopwatch", {{[el.target.name]: el.target.value}}) }});
    </script>
    </html>
    """
    return html
...
def stopwatch_press(request: HTTPRequest):
    print("stopwatch_button request")
    temp_time = str(1)
